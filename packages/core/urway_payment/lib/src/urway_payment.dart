...
import 'Model/PayRefundReq.dart';
import 'Model/PaySTC.dart';
import 'Model/PayTokenizeReq.dart';
...
class UrWayPayment {
  ...
  static Future<String> _read(BuildContext context, String country,
    ...
    text =
    await DefaultAssetBundle.of(context).loadString('assets/config/appconfig.json');
    ...
    if (isValidationSucess(
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (tar_url != null && tar_url.isEmpty==false) {


            //Todo check for ? if already there then don put
            if (tar_url.endsWith('?')==true) {
