...
import 'package:homemakers_merchant/app/shared/widgets/universal/async_builder/async_builder.dart';
import 'package:homemakers_merchant/bootstrap.dart';
import 'package:homemakers_merchant/bootup/injection_container.dart';
import '../connectivity_bloc/connectivity_service.dart';
import 'toast/colors.dart';
import 'toast/text_styles.dart';
import 'toast/types.dart';
import 'toast/widgets/error.dart';
import 'toast/widgets/info.dart';
import 'toast/widgets/normal.dart';
import 'toast/widgets/success.dart';
import 'toast/widgets/warning.dart';
import 'widgets/network_state.dart';

import 'widgets/notification.dart';

export 'toast/types.dart';

/// This will be used as a key for getting the [context] of [ConnectivityAppWrapper] widget
...
class ConnectivityAppWrapper extends StatefulWidget {
  ...
  @override
  State<ConnectivityAppWrapper> createState() => _ConnectivityAppWrapperState();
}
...
class _ConnectivityAppWrapperState extends State<ConnectivityAppWrapper> {
  ...
  Widget build(BuildContext context) {
    if (widget.showNetworkUpdates) {
      _listenToNetworkChanges(widget.bottomInternetNotificationPadding,
          messenger: widget.networkStateMessenger,
          connectivityCheckAddress: widget.connectivityCheckAddress);
    }
    return AsyncBuilder<InternetConnectivityState>(
        stream: serviceLocator<ConnectivityService>().offlineConnectivityStream,
        initial: InternetConnectivityState.internet,
        builder: (context, value) {
          return AbsorbPointer(
            absorbing: widget.disableInteraction &&
                (value != InternetConnectivityState.internet),
            child: SizedBox(
              key: _tKey,
              child: widget.child,
            ),
...
OverlayState? get _overlayState {
  ...
  assert(navigator != null,
      '''Cannot find ConnectivityAppWrapper above the widget tree, unable to show overlay''');
  return navigator!.overlay;
}

/// handling Internet Connectivity changes
void _listenToNetworkChanges(double bottomPadding,
    {String? connectivityCheckAddress,
    NetworkStateMessenger? messenger}) async {
  final ConnectivityService networkConnectivityService =
      serviceLocator<ConnectivityService>();
  networkConnectivityService.onConnectivityChanged().listen((state) {
    switch (state) {
      ...
      case ConnectivityResult.none ||
        ...
        _showNetworkStateWidget(
            NetworkState.Disconnected, messenger, bottomPadding);
        break;
    }
  });
}

void _checkInternetConnectionAndShowStatus(double bottomPadding,
    {String? connectivityCheckAddress,
    NetworkStateMessenger? messenger}) async {
  final ConnectivityService networkConnectivityService =
      serviceLocator<ConnectivityService>();
  try {
    networkConnectivityService.onInternetConnectivityChanged().listen((state) {
      if (state case InternetConnectivityState.connected) {
        _showNetworkStateWidget(NetworkState.Weak, messenger, bottomPadding);
      } else if (state case InternetConnectivityState.disconnected) {
        _showNetworkStateWidget(
            NetworkState.Disconnected, messenger, bottomPadding);
      } else {
        _showNetworkStateWidget(
            NetworkState.Connected, messenger, bottomPadding);
      }
    });
...
Future<void> _showNetworkStateWidget(NetworkState state,
    NetworkStateMessenger? messenger, double bottomPadding) async {
  try {
