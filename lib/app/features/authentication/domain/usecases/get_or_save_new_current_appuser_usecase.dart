    ...
    if (getCurrentUserResult.isNotNull) {
      await getCurrentUserResult.when(
        remote: (data, meta) async {
          appLog.d('Get current user remote ${data?.toMap()}');
          if (data != null) {
            ...
            saveCurrentUserResult.when(
              remote: (data, meta) {
                appLog.d('get or save user remote ${data?.toMap()}');
              },
              localDb: (data, meta) {
                appLog.d('get or save local ${data?.toMap()}');
              },
        ...
        localDb: (data, meta) async {
          appLog.d('Get current user local ${data?.toMap()}');
          if (data.isNull) {
            final saveCurrentUserResult = await serviceLocator<SaveAppUserUseCase>()(
              input,
            );
