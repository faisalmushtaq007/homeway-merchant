...
class AppUserEntity extends INetworkModel<AppUserEntity> with AppEquatable {
  ...
  factory AppUserEntity.fromMap(Map<String, dynamic> map) {
    return AppUserEntity(
      ...
      tokenCreationDateTime: (map['tokenCreationDateTime'] != null &&
          ...
          ? Timestamp.parse(map['tokenCreationDateTime'].toString())
              .toDateTime()
          : DateTime.now(),
      ...
      paymentBankEntities: (map['paymentBankEntities'] != null)
          ? map['paymentBankEntities']
              .map((e) => PaymentBankEntity.fromMap(e))
              .toList()
              .cast<PaymentBankEntity>()
          : <PaymentBankEntity>[],
      hasMultiplePaymentBanks: (map['hasMultiplePaymentBanks'] != null)
          ? map['hasMultiplePaymentBanks']
          : false,
      paymentBankEntity: (map['paymentBankEntity'] != null)
          ? PaymentBankEntity.fromMap(map['paymentBankEntity'])
          : PaymentBankEntity(),
      hasCurrentUser:
          (map['hasCurrentUser'] != null) ? map['hasCurrentUser'] : false,
  ...
  AppUserEntity copyWith({
    ...
    return AppUserEntity(
      ...
      hasMultiplePaymentBanks:
          hasMultiplePaymentBanks ?? this.hasMultiplePaymentBanks,
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'businessProfile': (businessProfile.isNotNull)
          ? this.businessProfile?.toMap()
          : BusinessProfileEntity().toMap(),
      'stores': (stores.isNotNullOrEmpty)
          ? this.stores.map((e) => e.toMap()).toList()
          : <StoreEntity>[],
      'token': this.token,
      'tokenCreationDateTime': Timestamp.fromDateTime(
          this.tokenCreationDateTime ?? DateTime.now().toUtc()),
      'hasUserAuthenticated': this.hasUserAuthenticated ?? false,
      'businessTypeEntity': (businessTypeEntity.isNotNull)
          ? this.businessTypeEntity?.toMap()
          : BusinessTypeEntity().toMap(),
      'currentProfileStatus': this.currentProfileStatus.name,
      'menus': (menus.isNotNullOrEmpty)
          ? this.menus.map((e) => e.toMap()).toList()
          : <MenuEntity>[],
      'drivers': (drivers.isNotNullOrEmpty)
          ? this.drivers.map((e) => e.toMap()).toList()
          : <StoreOwnDeliveryPartnersInfo>[],
      'addons': (addons.isNotNullOrEmpty)
          ? this.addons.map((e) => e.toMap()).toList()
          : <Addons>[],
      ...
      'paymentBankEntity': paymentBankEntity.isNotNull
          ? paymentBankEntity?.toMap()
          : PaymentBankEntity().toMap(),
      'hasMultiplePaymentBanks': hasMultiplePaymentBanks ?? false,
      'paymentBankEntities': (paymentBankEntities.isNotNullOrEmpty)
          ? this.paymentBankEntities.map((e) => e.toMap()).toList()
          : <PaymentBankEntity>[],
      'phoneNumberWithoutDialCode': this.phoneNumberWithoutDialCode ?? '',
    };
  }
  ...
  @override
  AppUserEntity fromJson(Map<String, dynamic> json) =>
      AppUserEntity.fromMap(json);
