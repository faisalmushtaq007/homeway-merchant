...
class UserPaymentBankLocalDbRepository<T extends PaymentBankEntity>
    implements BaseUserPaymentBankLocalDbRepository<PaymentBankEntity> {
  Future<Database> get _db async => AppDatabase.instance.database;

  StoreRef<int, Map<String, dynamic>> get _paymentBank =>
      AppDatabase.instance.paymentBank;

  @override
  Future<Either<RepositoryBaseFailure, PaymentBankEntity>> add(
      PaymentBankEntity entity) async {
  ...
  Future<Either<RepositoryBaseFailure, bool>> deleteByIdAndEntity(
      UniqueId uniqueId, PaymentBankEntity entity) async {
    // TODO(prasant): implement deleteByIdAndEntity
    throw UnimplementedError();
  }

  @override
  Future<Either<RepositoryBaseFailure, List<PaymentBankEntity>>>
      getAll() async {
  ...
  @override
  Future<Either<RepositoryBaseFailure, PaymentBankEntity?>> getById(
      UniqueId id) async {
    final result = await tryCatch<PaymentBankEntity?>(() async {
  ...
  @override
  Future<Either<RepositoryBaseFailure, PaymentBankEntity>> getByIdAndEntity(
      UniqueId uniqueId, PaymentBankEntity entity) async {
  ...
  @override
  Future<Either<RepositoryBaseFailure, PaymentBankEntity>> update(
      PaymentBankEntity entity, UniqueId uniqueId) async {
    final result = await tryCatch<PaymentBankEntity>(() async {
  ...
  @override
  Future<Either<RepositoryBaseFailure, PaymentBankEntity>> updateByIdAndEntity(
      UniqueId uniqueId, PaymentBankEntity entity) async {
  ...
  Future<Either<RepositoryBaseFailure, PaymentBankEntity>> upsert(
      {UniqueId? id,
      String? token,
      required PaymentBankEntity entity,
  ...
  Future<Map<String, RecordSnapshot<int, Map<String, Object?>>>>
      getPaymentBankEntityByIds(DatabaseClient db, List<int> ids) async {
    var snapshots = await _paymentBank.find(db,
        finder: Finder(
            filter: Filter.or(
                ids.map((e) => Filter.equals('paymentBankID', e)).toList())));
    return <String, RecordSnapshot<int, Map<String, Object?>>>{
      for (var snapshot in snapshots)
        snapshot.value['paymentBankID']!.toString(): snapshot
    };
  }
  ...
  Future<Either<RepositoryBaseFailure, List<PaymentBankEntity>>> saveAll({
    ...
    final result = await tryCatch<List<PaymentBankEntity>>(() async {
      ...
      return result.fold((l) {
        ...
        await db.transaction((transaction) async {
          var paymentBankIDs = convertOrderToMapObject
              .map((map) => map['paymentBankID'] as int)
              .toList();
          var map = await getPaymentBankEntityByIds(db, paymentBankIDs);
          // Watch for deleted item
          var keysToDelete =
              (await _paymentBank.findKeys(transaction)).toList();
