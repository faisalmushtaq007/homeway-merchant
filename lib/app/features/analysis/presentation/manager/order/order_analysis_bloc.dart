...
class OrderAnalysisBloc extends Bloc<OrderAnalysisEvent, OrderAnalysisState> {
  ...
  FutureOr<void> _todayOrderAnalysisEvent(
      TodayOrderAnalysisEvent event, Emitter<OrderAnalysisState> emit) async {
    if (event.analysisBy == AnalysisBy.todayOrder) {
      // local: readTodayOrderAnalysisData
      final TodayOrderAnalysisEntity todayOrderAnalysisData =
          await readTodayOrderAnalysisData();
      ...
      for (TodayOrderResult todayOrderResult in todayOrderAnalysisData.result) {
        ...
        final DayOrderStatus todayData =
            todayOrderResult.store.todayOrderStatus;
        final DayOrderStatus yesterdayDayData =
            todayOrderResult.store.yesterdayOrderStatus;
        chartData.add(ChartTodayEntity(storeAnalysisEntity.storeName,
            todayData.totalOrders, yesterdayDayData.totalOrders));
      ...
      final OverAllAnalysisData overAllData = OverAllAnalysisData(
        totalCustomers: overAllAnalysisData.fold(
            0, (sum, element) => sum + element.totalCustomers),
        totalStores: overAllAnalysisData.fold(
            0, (sum, element) => sum + element.totalStores),
        totalOrders: TotalOrders(
          countTotalOrders: overAllAnalysisData.fold(
              0, (sum, element) => sum + element.totalOrders.countTotalOrders),
          totalOrdersNew: overAllAnalysisData.fold(
              0, (sum, element) => sum + element.totalOrders.totalOrdersNew),
          deliver: overAllAnalysisData.fold(
            0,
            (sum, element) => sum + element.totalOrders.deliver,
          ),
        ),
        totalEarnings: overAllAnalysisData.fold(
            0, (sum, element) => sum + element.totalEarnings),
      );
      ...
      final TodaySalesAnalysisEntity todaySalesAnalysisData =
          await readTodaySalesAnalysisData();
      ...
      for (TodaySalesResult todaySalesResult in todaySalesAnalysisData.result) {
        final StoreSalesAnalysisEntity storeAnalysisEntity =
            todaySalesResult.store;
        final DaySalesStatus todayData =
            todaySalesResult.store.todaySalesStatus;
        final DaySalesStatus yesterdayDayData =
            todaySalesResult.store.yesterdaySalesStatus;
        chartData.add(ChartTodayEntity(storeAnalysisEntity.storeName,
            todayData.totalOrders, yesterdayDayData.totalOrders));
  ...
  FutureOr<void> _weeklyOrderAnalysisEvent(
      WeeklyOrderAnalysisEvent event, Emitter<OrderAnalysisState> emit) async {
    final WeeklyAnalysisEntity weeklyAnalysisData =
        await readWeeklyAnalysisData();
    ...
    final WeeklyAnalysisOverAllData weeklyOverallAnalysisData =
        weeklyResult.overAllData;
  ...
  FutureOr<void> _monthlyOrderAnalysisEvent(ByMonthlyOrderAnalysisEvent event,
      Emitter<OrderAnalysisState> emit) async {}
}
