...
class Place {
  ...
  Place.fromJSON(Map<String, dynamic> place, Geocoding? geocode) {
    try {
      this.description = place["description"] as String?;
      this.placeId = place["place_id"] as String?;
      this.types = place["types"] as List<dynamic>?;
  ...
  Future<Geolocation?>? get geolocation async {
    if (this._geolocation == null) {
      this._geolocation =
          await _geocode!.getGeolocation(description!) as Geolocation?;
      return _geolocation;
    }
    return _geolocation;
  }
