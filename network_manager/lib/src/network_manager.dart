...
import 'package:network_manager/src/model/error/file_manager_not_foud.dart';
import 'package:network_manager/src/operation/network_error_manager.dart';
import 'package:network_manager/src/utility/custom_logger.dart';
...
class NetworkManager<E extends INetworkModel<E>?> with dio.DioMixin implements INetworkManager<E> {
  ...
  NetworkManager({
    ...
    if (additionalInterceptors.isNotEmpty) {
      interceptors.addAll(additionalInterceptors.toList());
    }
    ...
    try {
      ...
      if (responseStatusCode >= HttpStatus.ok && responseStatusCode <= HttpStatus.multipleChoices) {
        ...
        if ((forceUpdateDecode ?? false) && _response0 is String) {
          _response0 = await _decodeBody(_response0);
        }
